...
if [[ $DELETE == "1" ]]; then
    while true; do
        read -rp "Delete ${DEPLOY_ENV}-frontend? [y/N] " delete_response
    ...
    # shellcheck disable=SC2086
    aws cloudformation delete-stack --stack-name "${DEPLOY_ENV}-frontend" --retain-resources $RESOURCES_TO_RETAIN
    aws cloudformation wait stack-delete-complete --stack-name "${DEPLOY_ENV}-frontend"
    echo "Stack deleted!"
fi
