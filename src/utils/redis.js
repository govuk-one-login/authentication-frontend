...
export async function getRedisConfig() {
    let redisHost = getRedisHost();
    let redisPort = getRedisPort();
    let redisPassword = getRedisPassword();
...
function getValueFromSSMResult(result, key) {
    const maybeValue = result.Parameters?.find((p) => p.Name === key)?.Value;
...
async function fetchParametersFromSSM(ssmParams) {
    const params = {
        Names: ssmParams,
        WithDecryption: true,
    };
