...
export function processIdentityRateLimitMiddleware(req, res, next) {
    ...
    if (initialRequestStart) {
        if (now.getTime() > initialRequestStart) {
            return res.redirect(createServiceRedirectErrorUrl(req.session.client.redirectUri, OIDC_ERRORS.ACCESS_DENIED, IPV_ERROR_CODES.IDENTITY_PROCESSING_TIMEOUT, req.session.client.state));
        }
    }
    else {
