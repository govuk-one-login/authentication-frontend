...
export function initialiseSessionMiddleware(req, res, next) {
    if (req.path === PATH_NAMES.AUTHORIZE) {
        req.session.client = {};
        const email = req.session && req.session.user ? req.session.user.email : undefined;
        const redactedPhoneNumber = req.session && req.session.user
            ? req.session.user.redactedPhoneNumber
            : undefined;
        const phoneNumber = req.session && req.session.user
...
export function getSessionIdMiddleware(req, res, next) {
...
export function validateSessionMiddleware(req, res, next) {
    ...
    const isReferrerInternal = getAppEnv() === "local"
        ? referrer?.includes("localhost")
        : referrer?.includes("gov.uk");
    if (isReferrerInternal) {
        req.log.info("request from gov.uk domain");
        return next(new ErrorWithLevel(ERROR_MESSAGES.INVALID_SESSION_GOV_UK_INTERNAL_REQUEST, ERROR_LOG_LEVEL.INFO));
    }
    return next(new ErrorWithLevel(ERROR_MESSAGES.INVALID_SESSION_NON_GOV_UK_EXTERNAL_REQUEST, ERROR_LOG_LEVEL.INFO));
}
