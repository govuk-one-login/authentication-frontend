...
export function enterPasswordGet(req, res) {
    const isReauthJourney = getJourneyTypeFromUserSession(req.session.user, {
...
export function enterSignInRetryBlockedGet(service = enterEmailService()) {
    return async function (req, res) {
        const email = req.session.user.email.toLowerCase();
        const { sessionId, clientSessionId, persistentSessionId } = res.locals;
        const checkUserIsBlockedResponse = await service.userExists(sessionId, email, clientSessionId, persistentSessionId, req);
        if (!checkUserIsBlockedResponse.success &&
...
export function enterPasswordAccountLockedGet(req, res) {
    res.render("enter-password/index-account-locked.njk", {
        newLink: "/sign-in-retry-blocked",
    });
}
export function enterPasswordAccountExistsGet(req, res) {
    const { email } = req.session.user;
    res.render(ENTER_PASSWORD_ACCOUNT_EXISTS_TEMPLATE, {
        email: email,
    });
}
export function enterPasswordPost(fromAccountExists = false, service = enterPasswordService(), mfaCodeService = mfaService(), accountInterventionsService = accountInterventionService()) {
    return async function (req, res) {
        const { email } = req.session.user;
        const { sessionId, clientSessionId, persistentSessionId } = res.locals;
        ...
        const userLogin = await service.loginUser(sessionId, email, req.body["password"], clientSessionId, persistentSessionId, req, journeyType);
        if (!userLogin.success) {
            ...
            if (errorCode === ERROR_CODES.SESSION_ID_MISSING_OR_INVALID) {
                req.log.warn(`Backend session is missing or invalid - user cannot enter password. Session id ${sessionId}`);
            ...
            return renderBadRequest(res, req, fromAccountExists
                ? ENTER_PASSWORD_ACCOUNT_EXISTS_TEMPLATE
                : ENTER_PASSWORD_TEMPLATE, error, { email });
        }
        ...
        if (isPasswordChangeRequired && supportAccountInterventions()) {
            const accountInterventionsResponse = await accountInterventionsService.accountInterventionStatus(sessionId, email, clientSessionId, persistentSessionId, req);
            if (accountInterventionsResponse.data.passwordResetRequired ||
                ...
                return res.redirect(await getNextPathAndUpdateJourney(req, req.path, USER_JOURNEY_EVENTS.COMMON_PASSWORD_AND_AIS_STATUS, null, sessionId));
            }
        }
        req.session.user.isSignInJourney = true;
        if (userLogin.data.mfaRequired &&
            userLogin.data.mfaMethodVerified &&
            userLogin.data.mfaMethodType === MFA_METHOD_TYPE.SMS &&
            !isPasswordChangeRequired) {
            const result = await mfaCodeService.sendMfaCode(sessionId, clientSessionId, email, persistentSessionId, false, xss(req.cookies.lng), req, journeyType);
            if (!result.success) {
        ...
        return res.redirect(await getNextPathAndUpdateJourney(req, req.path, USER_JOURNEY_EVENTS.CREDENTIALS_VALIDATED, {
    ...
    function handleMaxCredentialsReached(errorCode, journeyType, res, req) {
        if (journeyType != JOURNEY_TYPE.REAUTHENTICATION) {
            if (errorCode == ERROR_CODES.RE_AUTH_SIGN_IN_DETAILS_ENTERED_EXCEEDED) {
                throw new ReauthJourneyError("Reauth error code from login handler returned on a non-reauth journey");
            }
