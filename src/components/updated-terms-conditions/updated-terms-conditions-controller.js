...
import { UpdateType, } from "../common/update-profile/types";
import { updateProfileService } from "../common/update-profile/update-profile-service";
import { USER_JOURNEY_EVENTS } from "../common/state-machine/state-machine";
import { getNextPathAndUpdateJourney } from "../common/constants";
export function updatedTermsConditionsGet(req, res) {
    res.render("updated-terms-conditions/index.njk");
}
export function updatedTermsConditionsPost(service = updateProfileService()) {
    return async function (req, res) {
        ...
        if (termsAndConditionsResult === "accept") {
            const result = await service.updateProfile(sessionId, clientSessionId, email, {
                updateProfileType: UpdateType.UPDATE_TERMS_CONDS,
                profileInformation: true,
            }, persistentSessionId, req);
            if (!result.success) {
                throw new BadRequestError(result.data.message, result.data.code);
            }
            res.redirect(await getNextPathAndUpdateJourney(req, req.path, USER_JOURNEY_EVENTS.TERMS_AND_CONDITIONS_ACCEPTED, {
