...
export function checkAccountRecoveryPermitted(req, res, next) {
    if (req.session.user.isAccountRecoveryPermitted === true) {
        req.session.user.isAccountRecoveryJourney = true;
        getNextPathAndUpdateJourney(req, req.path, USER_JOURNEY_EVENTS.CHANGE_SECURITY_CODES_REQUESTED, null, res.locals.sessionId);
        return next();
    }
    ...
    throw new Error("Attempted to access /check-your-email without a valid request type");
}
