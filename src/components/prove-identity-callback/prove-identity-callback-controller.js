...
export function proveIdentityCallbackGetOrPost(service = proveIdentityCallbackService()) {
    return async function (req, res) {
        const { sessionId, clientSessionId, persistentSessionId } = res.locals;
        const clientName = req.session.client.name;
        const email = req.session.user.email;
        const response = await service.processIdentity(email, sessionId, clientSessionId, persistentSessionId, req);
        ...
        if (response.data.status === IdentityProcessingStatus.COMPLETED) {
            redirectPath = await service.generateSuccessfulRpReturnUrl(sessionId, clientSessionId, persistentSessionId, req);
        }
        else {
            redirectPath = createServiceRedirectErrorUrl(req.session.client.rpRedirectUri, OIDC_ERRORS.ACCESS_DENIED, IPV_ERROR_CODES.IDENTITY_PROCESSING_TIMEOUT, req.session.client.rpState);
        }
        return res.redirect(redirectPath);
    };
}
export function proveIdentityStatusCallbackGet(service = proveIdentityCallbackService()) {
...
export function proveIdentityCallbackSessionExpiryError(req, res) {
