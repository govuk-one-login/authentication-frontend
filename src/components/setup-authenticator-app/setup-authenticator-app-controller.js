...
import { JOURNEY_TYPE, MFA_METHOD_TYPE, NOTIFICATION_TYPE, } from "../../app.constants";
import xss from "xss";
...
import { isAccountRecoveryJourney, isAccountRecoveryJourneyAndEnabled, } from "../../utils/request";
const TEMPLATE = "setup-authenticator-app/index.njk";
export async function setupAuthenticatorAppGet(req, res) {
    const qrCodeText = generateQRCodeValue(req.session.user.authAppSecret, req.session.user.email, req.t("general.authenticatorAppIssuer"));
...
export function setupAuthenticatorAppPost(service = verifyMfaCodeService(), notificationService = sendNotificationService()) {
    return async function (req, res) {
        ...
        const verifyAccessCodeRes = await service.verifyMfaCode(MFA_METHOD_TYPE.AUTH_APP, code, sessionId, clientSessionId, persistentSessionId, req, journeyType, authAppSecret);
        if (!verifyAccessCodeRes.success) {
            if (verifyAccessCodeRes.data.code === ERROR_CODES.AUTH_APP_INVALID_CODE) {
                const error = formatValidationError("code", req.t("pages.setupAuthenticatorApp.code.validationError.invalidCode"));
                return renderBadRequest(res, req, TEMPLATE, error, {
                    qrCode: req.session.user.authAppQrCodeUrl,
                    secretKeyFragmentArray: splitSecretKeyIntoFragments(req.session.user.authAppSecret),
                });
            }
            throw new BadRequestError(verifyAccessCodeRes.data.message, verifyAccessCodeRes.data.code);
        }
        req.session.user.authAppSecret = null;
        req.session.user.phoneNumber = null;
        req.session.user.redactedPhoneNumber = null;
        const accountRecoveryEnabledJourney = isAccountRecoveryJourneyAndEnabled(req);
        ...
        await notificationService.sendNotification(res.locals.sessionId, res.locals.clientSessionId, req.session.user.email, notificationType, res.locals.persistentSessionId, xss(req.cookies.lng), req, journeyType);
        return res.redirect(await getNextPathAndUpdateJourney(req, req.path, USER_JOURNEY_EVENTS.MFA_CODE_VERIFIED, {
