import { defaultSmartAgentClient, } from "../../utils/smartAgent";
import { CONTACT_US_THEMES } from "../../app.constants";
export function prepareUserLocationTitle(residentInUK) {
    let tag = "";
...
export function prepareSecurityCodeSendMethodTitle(securityCodeSentMethod) {
...
export function prepareIdentityDocumentTitle(identityDocumentUsed) {
...
export function prepareProblemWithTitle(problemWith) {
...
export function getIdentifierTag(theme) {
...
export function getThemeTag(themes) {
    if (themes.theme !== CONTACT_US_THEMES.PROVING_IDENTITY_FACE_TO_FACE) {
        return `auth_${themes.theme}`;
    }
    return "";
}
export function getSubthemeTag(themes) {
    if (themes.subtheme) {
        return `auth_${themes.subtheme}`;
    }
    return "";
}
export function getRefererTag(contactForm) {
    if (contactForm.fromURL) {
        return `Referer obtained via Triage page fromURL: ${decodeURIComponent(contactForm.fromURL)}`;
    }
...
export function getContactUsService(smartAgentClient = defaultSmartAgentClient) {
    function formatSmartAgentMessageField(descriptions, optionalData, questions) {
        const message = [];
    ...
    function createSmartAgentCustomAttributes(contactForm) {
        ...
        customAttributes["sa-identity-document"] = prepareIdentityDocumentTitle(contactForm.identityDocumentUsed);
        ...
        customAttributes["sa-tag-location"] = prepareUserLocationTitle(contactForm.optionalData.location);
        ...
        customAttributes["sa-tag-permission-to-email"] = `Permission to email ${contactForm.feedbackContact ||
    ...
    const contactUsSubmitFormSmartAgent = async function (contactForm) {
        const payload = {
            email: contactForm.email,
            message: formatSmartAgentMessageField(contactForm.descriptions, contactForm.optionalData, contactForm.questions),
