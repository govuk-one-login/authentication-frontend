...
export function verifyCodePost(service, accountInterventionsService, options) {
    return async function (req, res) {
        const email = req.session.user.email.toLowerCase();
        const code = req.body["code"];
        const { sessionId, clientSessionId, persistentSessionId } = res.locals;
        const result = await service.verifyCode(sessionId, code, options.notificationType, clientSessionId, persistentSessionId, req, options.journeyType);
        if (!result.success) {
            ...
            if (ERROR_CODES.ENTERED_INVALID_VERIFY_EMAIL_CODE_MAX_TIMES ===
                result.data.code &&
        ...
        if (supportAccountInterventions() &&
            ...
            if (nextEvent === USER_JOURNEY_EVENTS.EMAIL_SECURITY_CODES_CODE_VERIFIED ||
                (nextEvent === USER_JOURNEY_EVENTS.MFA_CODE_VERIFIED &&
        ...
        res.redirect(await getNextPathAndUpdateJourney(req, req.path, nextEvent, {
            ...
            isOnForcedPasswordResetJourney: req.path === PATH_NAMES.RESET_PASSWORD_CHECK_EMAIL &&
                req.session.user.withinForcedPasswordResetJourney,
