import { COOKIE_CONSENT, COOKIES_PREFERENCES_SET, PATH_NAMES, ERROR_LOG_LEVEL, COOKIES_CHANNEL, CHANNEL, } from "../../app.constants";
...
import { isValidChannel, getDefaultChannel, getOrchToAuthExpectedClientId, supportReauthentication, getOrchStubToAuthExpectedClientId, } from "../../config";
import { logger } from "../../utils/logger";
import { isReauth, isUpliftRequired } from "../../utils/request";
export function authorizeGet(authService = authorizeService(), cookiesConsentService = cookieConsentService(), kmsService = new KmsDecryptionService(), jwtService = new JwtService()) {
    return async function (req, res) {
        const { sessionId, clientSessionId, persistentSessionId } = res.locals;
        const loginPrompt = sanitize(req.query.prompt);
        const clientId = req.query.client_id;
        const responseType = req.query.response_type;
        let claims;
        try {
            validateQueryParams(clientId, responseType);
            const encryptedAuthRequestJWE = req.query.request;
            const authRequestJweDecryptedAsJwt = await kmsService.decrypt(encryptedAuthRequestJWE);
            claims = await jwtService.getPayloadWithValidation(authRequestJweDecryptedAsJwt);
        }
        catch (error) {
            throw new BadRequestError(error.message);
        }
        const startAuthResponse = await authService.start(sessionId, clientSessionId, persistentSessionId, req, {
            authenticated: claims.authenticated,
            current_credential_strength: claims.current_credential_strength,
            previous_session_id: claims.previous_session_id,
            previous_govuk_signin_journey_id: claims.previous_govuk_signin_journey_id,
            reauthenticate: claims.reauthenticate,
        });
        if (!startAuthResponse.success) {
            const startError = new BadRequestError(startAuthResponse.data.message, startAuthResponse.data.code);
            if (startAuthResponse.data.code &&
        ...
        if (supportReauthentication() &&
            ...
            logger.info(`Start response indicates user with session ${res.locals.sessionId} is blocked for reauth, redirecting back to orchestration`);
        ...
        let redirectPath = await getNextPathAndUpdateJourney(req, PATH_NAMES.AUTHORIZE, nextStateEvent, {
        ...
        if (req.session.user.channel === CHANNEL.STRATEGIC_APP) {
            const consentCookieValue = cookiesConsentService.createConsentCookieValue(COOKIE_CONSENT.REJECT);
            ...
            const consentCookieValue = cookiesConsentService.createConsentCookieValue(cookieConsent);
            res.cookie(COOKIES_PREFERENCES_SET, consentCookieValue.value, {
            ...
            if (startAuthResponse.data.user.gaCrossDomainTrackingId &&
        ...
        const faceToFaceRpGoogleAnalyticsParamValue = sanitize(req.query[faceToFaceRpGoogleAnalyticsParamKey]);
        return res.redirect(appendQueryParamIfHasValue(redirectPath, faceToFaceRpGoogleAnalyticsParamKey, faceToFaceRpGoogleAnalyticsParamValue));
    };
}
function setSessionDataFromClaims(req, claims) {
...
function setSessionDataFromAuthResponse(req, startAuthResponse) {
...
function validateQueryParams(clientId, responseType) {
...
function persistSessionDataInCookies(req, res) {
