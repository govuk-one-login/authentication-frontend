import { JOURNEY_TYPE, NOTIFICATION_TYPE, WEB_TO_MOBILE_ERROR_MESSAGE_MAPPINGS, } from "../../app.constants";
...
export function enterEmailGet(req, res) {
...
export function enterEmailCreateGet(req, res) {
    return res.render("enter-email/index-create-account.njk");
}
export function enterEmailPost(service = enterEmailService(), checkReauthService = checkReauthUsersService()) {
    return async function (req, res) {
        const email = req.body.email;
        const { sessionId, clientSessionId, persistentSessionId } = res.locals;
        req.session.user.email = email.toLowerCase();
        if (isReauth(req)) {
            const checkReauth = await checkReauthService.checkReauthUsers(sessionId, email, req.session.user.reauthenticate, clientSessionId, persistentSessionId, req);
            if (!checkReauth.success) {
                if (isLocked(req.session.user.wrongEmailEnteredLock)) {
                    return res.render(BLOCKED_TEMPLATE);
                }
                const CHANNEL_SPECIFIC_EMAIL_ERROR_KEY = getChannelSpecificErrorMessage(EMAIL_ERROR_KEY, req.body.isStrategicAppReauth === "true", WEB_TO_MOBILE_ERROR_MESSAGE_MAPPINGS);
        ...
        const result = await service.userExists(sessionId, email, res.locals.clientSessionId, res.locals.persistentSessionId, req);
        if (!result.success) {
        ...
        if (result.data.lockoutInformation != null &&
        ...
        return res.redirect(await getNextPathAndUpdateJourney(req, req.path, nextState, null, sessionId));
    };
}
export function enterEmailCreatePost(service = enterEmailService(), notificationService = sendNotificationService()) {
    return async function (req, res) {
        const email = req.body.email.toLowerCase();
        const { sessionId, clientSessionId, persistentSessionId } = res.locals;
        req.session.user.email = email;
        const userExistsResponse = await service.userExists(sessionId, email, clientSessionId, persistentSessionId, req);
        if (!userExistsResponse.success) {
            throw new BadRequestError(userExistsResponse.data.message, userExistsResponse.data.code);
        }
        if (userExistsResponse.data.doesUserExist) {
            return res.redirect(await getNextPathAndUpdateJourney(req, req.path, USER_JOURNEY_EVENTS.ACCOUNT_FOUND_CREATE, null, sessionId));
        }
        const sendNotificationResponse = await notificationService.sendNotification(sessionId, clientSessionId, email, NOTIFICATION_TYPE.VERIFY_EMAIL, persistentSessionId, xss(req.cookies.lng), req, JOURNEY_TYPE.REGISTRATION);
        ...
        return res.redirect(await getNextPathAndUpdateJourney(req, req.path, USER_JOURNEY_EVENTS.SEND_EMAIL_CODE, null, sessionId));
    };
}
function handleBadRequest(req, res, errorMessageKey) {
...
function setUpAuthAppLocks(req, lockoutArray) {
