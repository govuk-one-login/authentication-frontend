...
export function validateEnterEmailRequest(template = "enter-email/index-existing-account.njk") {
    return [
        body("email")
            .notEmpty()
            ...
            .withMessage((value, { req }) => {
            return req.t("pages.enterEmailExistingAccount.email.validationError.email", { value });
        })
            ...
            if (req.session.user.reauthenticate) {
                const errorMessage = getChannelSpecificErrorMessage("pages.reEnterEmailAccount.enterYourEmailAddressError", req.body.isStrategicAppReauth === "true", WEB_TO_MOBILE_ERROR_MESSAGE_MAPPINGS);
