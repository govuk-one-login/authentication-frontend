...
import { getCodeEnteredWrongBlockDurationInMinutes, routeUsersToNewIpvJourney, supportAccountRecovery, supportMfaResetWithIpv, supportReauthentication, } from "../../config";
...
export const ENTER_AUTH_APP_CODE_DEFAULT_TEMPLATE_NAME = "enter-authenticator-app-code/index.njk";
export const UPLIFT_REQUIRED_AUTH_APP_TEMPLATE_NAME = "enter-authenticator-app-code/index-2fa-service-uplift-auth-app.njk";
export function enterAuthenticatorAppCodeGet(acctRecoveryService = accountRecoveryService()) {
    return async function (req, res) {
        ...
        const accountRecoveryResponse = await acctRecoveryService.accountRecovery(sessionId, clientSessionId, email, persistentSessionId, req);
        if (!accountRecoveryResponse.success) {
            throw new BadRequestError(accountRecoveryResponse.data.message, accountRecoveryResponse.data.code);
        }
        const isAccountRecoveryPermittedForUser = (req.session.user.isAccountRecoveryPermitted =
            accountRecoveryResponse.data.accountRecoveryPermitted);
        const routeUserViaIpvReset = supportMfaResetWithIpv() && routeUsersToNewIpvJourney();
        const mfaResetPath = routeUserViaIpvReset
            ? PATH_NAMES.MFA_RESET_WITH_IPV
            : pathWithQueryParam(PATH_NAMES.CHECK_YOUR_EMAIL_CHANGE_SECURITY_CODES, "type", MFA_METHOD_TYPE.AUTH_APP);
        return res.render(templateName, {
...
function handleReauthFailure(req, res) {
    if (req.session.client?.redirectUri) {
        return res.redirect(req.session.client.redirectUri.concat("?error=login_required"));
    }
    else {
        throw new ReauthJourneyError("Re-auth journey failed due to missing redirect uri in client session.");
    }
}
export const enterAuthenticatorAppCodePost = (service = verifyMfaCodeService()) => {
    return async function (req, res) {
        const { sessionId, clientSessionId, persistentSessionId } = res.locals;
        ...
        const result = await service.verifyMfaCode(MFA_METHOD_TYPE.AUTH_APP, req.body["code"], sessionId, clientSessionId, persistentSessionId, req, journeyType);
        if (!result.success) {
            return handleAuthAppCodePostError(req, res, template, journeyType, result);
        }
        res.redirect(await getNextPathAndUpdateJourney(req, req.path, USER_JOURNEY_EVENTS.AUTH_APP_CODE_VERIFIED, {
            isIdentityRequired: req.session.user.isIdentityRequired,
            isLatestTermsAndConditionsAccepted: req.session.user.isLatestTermsAndConditionsAccepted,
        }, res.locals.sessionId));
    };
};
function handleAuthAppCodePostError(req, res, template, journeyType, result) {
    const error = result.data.code;
    if (error === ERROR_CODES.AUTH_APP_INVALID_CODE) {
        const error = formatValidationError("code", req.t("pages.enterAuthenticatorAppCode.code.validationError.invalidCode"));
        return renderBadRequest(res, req, template, error);
    }
    const isReauthJourneyInEnvWithReauthConfigured = supportReauthentication() && journeyType == JOURNEY_TYPE.REAUTHENTICATION;
    if (error === ERROR_CODES.AUTH_APP_INVALID_CODE_MAX_ATTEMPTS_REACHED) {
        ...
        req.session.user.wrongCodeEnteredLock = new Date(Date.now() + getCodeEnteredWrongBlockDurationInMinutes() * 60000).toUTCString();
    }
    if (error === ERROR_CODES.RE_AUTH_SIGN_IN_DETAILS_ENTERED_EXCEEDED) {
        if (isReauthJourneyInEnvWithReauthConfigured) {
            ...
            throw new ReauthJourneyError("Reauth error response returned on a non reauth journey");
        }
    }
