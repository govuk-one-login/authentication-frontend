...
const RESEND_CODE_LINK = pathWithQueryParam(PATH_NAMES.RESEND_MFA_CODE_ACCOUNT_CREATION, "isResendCodeRequest", "true");
export function checkYourPhoneGet(req, res) {
...
export const checkYourPhonePost = (service = verifyMfaCodeService(), notificationService = sendNotificationService()) => {
    return async function (req, res) {
        ...
        const result = await service.verifyMfaCode(MFA_METHOD_TYPE.SMS, req.body["code"], sessionId, clientSessionId, persistentSessionId, req, journeyType, req.session.user.phoneNumber);
        if (!result.success) {
            if (result.data.code === ERROR_CODES.INVALID_VERIFY_PHONE_NUMBER_CODE) {
                const error = formatValidationError("code", req.t("pages.checkYourPhone.code.validationError.invalidCode"));
        ...
        return res.redirect(await getNextPathAndUpdateJourney(req, req.path, USER_JOURNEY_EVENTS.PHONE_NUMBER_VERIFIED, {
