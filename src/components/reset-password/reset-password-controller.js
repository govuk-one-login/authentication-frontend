...
export function resetPasswordRequiredGet(req, res) {
    res.render(resetPasswordTemplate, {
        isPasswordChangeRequired: req.session.user.isPasswordChangeRequired,
    });
}
export function resetPasswordPost(resetService = resetPasswordService(), loginService = enterPasswordService()) {
    return async function (req, res) {
        ...
        const allowMfaResetAfterPasswordReset = supportMfaResetWithIpv() && routeUsersToNewIpvJourney();
        const updatePasswordResponse = await resetService.updatePassword(newPassword, sessionId, clientSessionId, persistentSessionId, withinForcedPasswordResetJourney, allowMfaResetAfterPasswordReset, req);
        if (!updatePasswordResponse.success) {
            if (updatePasswordResponse.data.code ===
                ERROR_CODES.NEW_PASSWORD_SAME_AS_EXISTING) {
                const error = formatValidationError("password", req.t("pages.resetPassword.password.validationError.samePassword"));
        ...
        const loginResponse = await loginService.loginUser(sessionId, email, newPassword, clientSessionId, persistentSessionId, req);
...
export async function resetPasswordRequestGet(req, res) {
    return res.redirect(await getNextPathAndUpdateJourney(req, req.path, USER_JOURNEY_EVENTS.PASSWORD_RESET_REQUESTED, null, res.locals.sessionId));
}
